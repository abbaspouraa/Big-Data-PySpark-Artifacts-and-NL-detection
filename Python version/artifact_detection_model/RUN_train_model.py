...
from artifact_detection_model.utils.paths import PROJECT_ROOT, REVIEWER_1_PATH, REVIEWER_2_PATH, Amir_Ali_Misagh_PATH
...
def main():
    ...
    r3data, r3target = get_manual_validation_data_set(Amir_Ali_Misagh_PATH)

    train_frac = 0.4
    df_train, df_test = get_training_and_test_set()
    df_train = df_train.sample(frac=train_frac, random_state=42)
    report, pipeline = run_ml_artifact_training(df_train, df_test, LinearSVC(random_state=42, max_iter=1000), model_output_path=OUT_PATH)

    report.update({'name': 'LSVCdef'})
    report.update({'train_frac': train_frac})

    report.update(validation_performance_on_dataset(pipeline, r1data, r1target, 'reviewer_1',  output_misclassified=OUT_PATH))
    report.update(validation_performance_on_dataset(pipeline, r2data, r2target, 'reviewer_2', output_misclassified=OUT_PATH))
    report.update(validation_performance_on_dataset(pipeline, r3data, r3target, 'Amir_Ali_Misagh', output_misclassified=OUT_PATH))
